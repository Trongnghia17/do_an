...
export default function OnlineExamLibrary() {
  ...
  const [filters, setFilters] = useState({
    skillType: [],
    taskType: [], // Writing Task 1, Task 2, Speaking Part 1-3, etc.
    questionType: [], // Line Graph, Bar Chart, etc. for Writing
    section: [], // Section 1-4 for Listening
    passage: [], // Passage 1-3 for Reading
  });
  ...
  const filteredSkills = skills.filter(skill => {
    ...
    // Task type filter (Writing Task 1/2, Speaking Part 1-3, etc.)
    const matchesTaskType = filters.taskType.length === 0 ||
      filters.taskType.some(taskType => {
        const skillName = skill.name?.toLowerCase() || '';
        return skillName.includes(taskType.toLowerCase());
      });

    // Question type filter (for Writing: Line Graph, Bar Chart, etc.)
    const matchesQuestionType = filters.questionType.length === 0 ||
      filters.questionType.some(questionType => {
        const skillName = skill.name?.toLowerCase() || '';
        const skillDescription = skill.description?.toLowerCase() || '';
        return skillName.includes(questionType.toLowerCase()) || 
               skillDescription.includes(questionType.toLowerCase());
      });

    // Section filter (for Listening)
    const matchesSection = filters.section.length === 0 ||
      filters.section.some(section => {
        const skillName = skill.name?.toLowerCase() || '';
        return skillName.includes(section.toLowerCase());
      });

    // Passage filter (for Reading)
    const matchesPassage = filters.passage.length === 0 ||
      filters.passage.some(passage => {
        const skillName = skill.name?.toLowerCase() || '';
        return skillName.includes(passage.toLowerCase());
      });

    // Task filter (from header)
    if (selectedTask !== 'all') {
    ...
    return matchesSearch && matchesSkillType && matchesTaskType && 
           matchesQuestionType && matchesSection && matchesPassage;
  });
                ...
                <div className="sidebar-item__content">
                  <div className="sidebar-subitem">
                    ...
                    <ul className="sidebar-subitem__list">
                      <li 
                        className={filters.questionType.includes('Line Graph') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Line Graph')}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                          <path d="M2.5 2.5V15.8333C2.5 16.2754 2.67559 16.6993 2.98816 17.0118C3.30072 17.3244 3.72464 17.5 4.16667 17.5H17.5" stroke="#454545" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                          <path d="M15.8335 7.5L11.6668 11.6667L8.3335 8.33333L5.8335 10.8333" stroke="#454545" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Line Graph
                      </li>
                      <li 
                        className={filters.questionType.includes('Bar Chart') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Bar Chart')}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                          <path d="M5.00007 10.0003H3.43757C2.93757 10.0003 2.53174 10.4062 2.53174 10.9062V16.8028C2.53174 17.3028 2.93757 17.7087 3.43757 17.7087H4.99841C5.49841 17.7087 5.90424 17.3028 5.90424 16.8028V10.9062C5.90424 10.4062 5.49841 10.0003 4.99841 10.0003M10.7809 6.14616H9.22007C8.72007 6.14616 8.31424 6.55199 8.31424 7.05199V16.8028C8.31424 17.3028 8.71924 17.7087 9.21924 17.7087H10.7809C11.2809 17.7087 11.6867 17.3028 11.6867 16.8028V7.05199C11.6867 6.55199 11.2809 6.14616 10.7809 6.14616ZM16.5617 2.29199H15.0009C14.5009 2.29199 14.0951 2.69783 14.0951 3.19783V16.8028C14.0951 17.3028 14.5009 17.7087 15.0009 17.7087H16.5617C17.0617 17.7087 17.4676 17.3028 17.4676 16.8028V3.19783C17.4676 2.69783 17.0617 2.29199 16.5617 2.29199Z" stroke="#454545" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                        Bar Chart
                      </li>
                      <li 
                        className={filters.questionType.includes('Pie Chart') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Pie Chart')}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                      ...
                      <li 
                        className={filters.questionType.includes('Table') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Table')}
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
              ...
              {expandedSections.writingTask2 && (
                <div className="sidebar-item__content">
                  <div className="sidebar-subitem">
                    ...
                    <ul className="sidebar-subitem__list">
                      <li 
                        className={filters.questionType.includes('Agree or Disagree') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Agree or Disagree')}
                      >
                        Agree or Disagree
                      </li>
                      <li 
                        className={filters.questionType.includes('Discussion') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Discussion')}
                      >
                        Discussion
                      </li>
                      <li 
                        className={filters.questionType.includes('Advantages and Disadvantages') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Advantages and Disadvantages')}
                      >
                        Advantages and Disadvantages
                      </li>
                      <li 
                        className={filters.questionType.includes('Causes, Problems and Solutions') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Causes, Problems and Solutions')}
                      >
                        Causes, Problems and Solutions
                      </li>
                      <li 
                        className={filters.questionType.includes('Two-Part Question') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Two-Part Question')}
                      >
                        Two-Part Question
                      </li>
                      <li 
                        className={filters.questionType.includes('Positive or Negative Development') ? 'active' : ''}
                        onClick={() => handleFilterChange('questionType', 'Positive or Negative Development')}
                      >
              ...
              {expandedSections.listeningBaiLe && (
                <div className="sidebar-item__content">
                  <div className="sidebar-subitem">
                    ...
                    <ul className="sidebar-subitem__list">
                      <li 
                        className={filters.section.includes('Section 1') ? 'active' : ''}
                        onClick={() => handleFilterChange('section', 'Section 1')}
                      >
                        Section 1
                      </li>
                      <li 
                        className={filters.section.includes('Section 2') ? 'active' : ''}
                        onClick={() => handleFilterChange('section', 'Section 2')}
                      >
                        Section 2
                      </li>
                      <li 
                        className={filters.section.includes('Section 3') ? 'active' : ''}
                        onClick={() => handleFilterChange('section', 'Section 3')}
                      >
                        Section 3
                      </li>
                      <li 
                        className={filters.section.includes('Section 4') ? 'active' : ''}
                        onClick={() => handleFilterChange('section', 'Section 4')}
                      >
                        Section 4
                      </li>
                    </ul>
              ...
              {expandedSections.readingBaiLe && (
                <div className="sidebar-item__content">
                  <div className="sidebar-subitem">
                    ...
                    <ul className="sidebar-subitem__list">
                      <li 
                        className={filters.passage.includes('Passage 1') ? 'active' : ''}
                        onClick={() => handleFilterChange('passage', 'Passage 1')}
                      >
                        Passage 1
                      </li>
                      <li 
                        className={filters.passage.includes('Passage 2') ? 'active' : ''}
                        onClick={() => handleFilterChange('passage', 'Passage 2')}
                      >
                        Passage 2
                      </li>
                      <li 
                        className={filters.passage.includes('Passage 3') ? 'active' : ''}
                        onClick={() => handleFilterChange('passage', 'Passage 3')}
                      >
                        Passage 3
                      </li>
                    </ul>
            ...
            <>
              ...
              <div className="results-info">
                <span className="results-count">
                  Tìm thấy <strong>{sortedSkills.length}</strong> bài thi
                </span>
              </div>

              {sortedSkills.length === 0 ? (
                <div className="online-exam-library__empty">Không có bài thi nào phù hợp với bộ lọc</div>
              ) : (
                <>
